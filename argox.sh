...
E[0]="Language:\n 1. English (default) \n 2. 简体中文"
C[0]="${E[0]}"
E[1]="1. Add the option of blocking on returning to China; 2. Add a number of quality cdn's that are collected online."
C[1]="1. 增加禁止归国选项; 2. 增加线上收录的若干优质 cdn"
...
E[49]="Enable the blocking on returning to China. This will prevent you from accessing mainland websites and the server IP will not be logged by the website, the default is: [not need to turn on], press [y] if needed:"
C[49]="开启禁止回国。这将访问不了大陆网站，同时也不会被网站记录服务器 IP，默认为: [不需要开启]，如需要请按 [y]:"
E[50]="Blocking on returning to China"
C[50]="禁止回国"
...
EOF
  ...
  # 禁止回国开启时，修改路由规则文件
  [[ "$RETURN" =~ 'y' ]] && sed -i "/outboundTag/s/direct/block/g" $WORK_DIR/outbound.json
...
switch_return() {
  if grep -q 'outboundTag.*direct' $WORK_DIR/outbound.json; then
    sed -i "/outboundTag/s/direct/block/g" $WORK_DIR/outbound.json
  else
    sed -i "/outboundTag/s/block/direct/g" $WORK_DIR/outbound.json
  fi
